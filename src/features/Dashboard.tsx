...
export function Dashboard({ state }: DashboardProps) {
  ...
  const draftOrders = state.sales.orders.filter((o: any) => o.status === "DRAFT").length;
  const shippedOrders = state.sales.orders.filter((o: any) => o.status === "SHIPPED").length;
  const invoiceCount = state.finance.invoices.length;
  const auditCount = state.auditTrail.length;

  const orderValue = state.sales.orders.reduce((sum: number, o: any) => sum + o.totalAmount, 0);

  return (
    <div>
      <h2>Dashboard</h2>
      <p style={{ color: "#666", marginBottom: "24px" }}>
        Welcome to MiniFlow ERP - a learning project demonstrating ERP architecture.
      </p>
      ...
      >
        <h3 style={{ marginTop: "0", color: "#005a87" }}>🎓 ERP Concepts in MiniFlow</h3>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
          <div>
            ...
            <ul style={{ color: "#333", lineHeight: "1.8" }}>
              ...
              <li>
                <strong>Inventory:</strong> Stock tracking, reservations, warehouses
              </li>
              <li>
                <strong>Finance:</strong> Invoices, payments, accounting
              </li>
              <li>
                <strong>Catalogs:</strong> Master data (products, customers, terms)
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul style={{ color: "#333", lineHeight: "1.8" }}>
              <li>
                <strong>Configuration:</strong> Status transitions, roles, allowed actions
              </li>
              <li>
                <strong>Side Effects:</strong> Stock reservation, invoice creation
              </li>
              <li>
                <strong>Audit Trail:</strong> Every action recorded with timestamp
              </li>
              <li>
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
          <div>
            ...
            <p style={{ color: "#666", fontSize: "0.9rem" }}>
              Defines all workflow rules: statuses, actions, roles, transitions. Add
              new statuses here, UI updates automatically.
            </p>
          </div>
          ...
          <div>
            ...
            <p style={{ color: "#666", fontSize: "0.9rem" }}>
              Implements side effects: stock reservation, invoice creation, audit
              logging. Hard-coded business logic lives here.
            </p>
          </div>
      ...
      >
        ...
        <p style={{ color: "#333", lineHeight: "1.6" }}>
          Start by creating a sales order in the <strong>Sales</strong> tab. You'll see
          how the workflow configuration controls what actions are available. Try different
          roles to understand permissions. Then watch how domains interact: reserve stock
          in Inventory, see invoices auto-created in Finance, and check the audit trail.
        </p>
      </div>
    </div>
  );
